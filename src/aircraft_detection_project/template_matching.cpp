...
{
    const std::filesystem::path avg_airplanes_dir(std::filesystem::path(TRAINING_DATASET_PATH).parent_path() / "avg_airplanes");
...
/*
std::vector<cv::Point> findTemplateMatches(const cv::Mat& src_img, const std::vector<cv::Mat>& avg_planes)
{
    ...
    {
        for (int degree_angle = 0; degree_angle < 360; degree_angle += angle_step)
        {
            ...
            cv::Mat NCC_Output;
            cv::matchTemplate(dst, avg_plane, NCC_Output, cv::TM_CCOEFF_NORMED);
            ...
            // Utilizzare cv::transform direttamente su un punto singolo
            std::vector<cv::Point2f> pointsOriginal(1);
            cv::transform(std::vector<cv::Point2f>{matchCenter}, pointsOriginal, inv_rotation_mat);

            cv::Point matchCenterOriginal = pointsOriginal[0];

            // Memorizzazione del risultato
            matched_points.push_back(matchCenterOriginal);
        }
    }
...
auto angle_range(int start, int end, int step)
{
...
std::vector<cv::Point> templateMatching(const cv::Mat& src_img)
{
    ...
    std::vector<cv::Point> max_corr_points = filterPointsByMinDistance(matched_points, 20);

    return max_corr_points;
}