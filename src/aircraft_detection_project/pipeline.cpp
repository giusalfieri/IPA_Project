...
void classify(const std::string& testing_img_id)
{
    ...
    // rois_labels_for_points_inside_yolo = labelRoisBasedOnIouWithYoloBoxes(yoloBox_roi_pairs, iou_threshold); DA RIMUOVERE
    ...
    saveRoiLabelPairsToFile(rois_label_pairs_all_points, (std::filesystem::path(SRC_DIR_PATH) / "roi_label_pairs.csv").string());
    // Extract ROIs
    ...
    std::filesystem::path output_dir = createDirectory(std::filesystem::path(SRC_DIR_PATH), "svm_inference_input");
    ...
    cv::imwrite((std::filesystem::path(SRC_DIR_PATH) / (testing_img_id + "_result.png") ).string(), src_img_drawing);
